...
def start(conversions,length,len,check) 
    check = ["length"]
    ...
    unit2 = gets.chomp
        ...
        result = convert_length(num,unit1,unit2,form,conversions,length,con) #change this when adding more conversions 
        puts result
end 

def convert_length(num,u1,u2,form,conversions,order,con)
    unit1 = order[con][u1]
    unit2 = order[con][u2]
    list = conversions[con][u2][u1] #number for conversion
